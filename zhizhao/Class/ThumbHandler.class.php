 ...
 *
 * @author 偶然
 * @version $Id: ThumbHandler.class.php 40 2014-01-09 02:49:50Z YinHailin $
 * @package system
 */
...
class ThumbHandler
{
	...
	var $thumbnail_w; //补白缩略图宽度
	
	var $thumbnail_h; //补白缩略图高度
	
	var $fill_color = '#ffffff'; //填充颜色
	
	var $file_alpha = 0; //填充透明度
    ...
    function createImg($a, $b=null)
    {
        ...
        if(2 == $num)
        {
            ...
            if(0 == $w)
            {
                return false; //die("目标宽度不能为0");
            }
            if(0 == $h)
            {
                return false; //die("目标高度不能为0");
            }
    ...
    function _output()
    {
        ...
        if(function_exists($func_name))
        {
			...
			$newImage=imagecreatetruecolor($this->thumbnail_w,$this->thumbnail_h);
			$fillColor = $this->_parseColor($this->fill_color);
			$x = ($this->thumbnail_w - $this->dst_w)/2;
			$y = ($this->thumbnail_h - $this->dst_h)/2;
			
			$alpha = imagecolorallocatealpha($newImage , $fillColor[0], $fillColor[1], $fillColor[2], $this->file_alpha);//拾取一个完全透明的颜色，不要用imagecolorallocate拾色      
			imagealphablending($newImage , false);//关闭混合模式，以便透明颜色能覆盖原画板    
			imagefill($newImage , 0 , 0 , $alpha);//填充     
			imagesavealpha($newImage , true);//设置保存PNG时保留透明通道信息    
			imagecopy($newImage, $this->h_dst,$x,$y,0,0,$this->dst_w,$this->dst_h);
            imagepng($newImage, $this->dst_img); 
        }
        else
        {
            return false;
    ...
    function _setNewImgSize($img_w, $img_h=null)
    {
        ...
        if(2 == $num)
        {
            ...
            {
                return false; //die("图片边框过大，已超过了图片的宽度");
            }
    ...
    function _checkMaskValid()
    {
        ...
        {
            return false; //die("水印图片尺寸大于原图，请缩小水印图");
        }
    }
    ...
    function _getImgType($file_path)
    {
        ...
        {
            return false; //die("文件不存在,不能取得文件类型!");
        }
    }
	...
	/*
	 * 设置填充颜色
     * @param    string     $fillColor    颜色值（16进制）
	 */
	 ...
	 * @param int $fillAlpha 
	 */
	function setFillAlpha($fileAlpha) {
		$this->file_alpha = $fileAlpha;	
	}
}
?>