	...
	class TravelAction extends CommonAction{
		...
		public function runAddSort(){
			...
			if(M('travel_sort')->add($_POST)){
				$this->success('添加成功',U(GROUP_NAME . '/Travel/sortlist'));	
			}else{
				$this->error('添加失败');	
			}
		}
		...
		public function editSort(){
			...
			if($_POST){
				...
				$result = $db->save($data);
				if($result){
					$this->success('操作成功',U(GROUP_NAME .'/Travel/sortlist'));
				}else{
					$this->error('操作失败');	
				}
			}else{
		...
		public function deleteSort(){
			...
			if($db = M('travel_sort')->where(array('id in('.$idList.')'))->delete()){
				$this->success('删除成功',U(GROUP_NAME .'/Travel/sortlist'));
			}else{
				$this->error('删除失败');
			}
		}
...
/*-------------------------------------------------以上分类页  以下详情页----------------------------------------------------*/
		...
		public function runAddDetails(){
			...
			if($db->data($data)->add()){
				$this->success('添加成功',U(GROUP_NAME.'/Travel/detailslist'));	
			}else{
				$this->error('添加失败');	
			}
		}
		...
		public function editDetails(){
			if($_POST){
				...
				if($db->data($data)->save()){
					$this->success('修改成功',U(GROUP_NAME.'/Travel/detailslist'));	
				}else{
					$this->error('修改失败');	
				}
			}else{
		...
		public function deleteDetails(){
			...
			$this->success('删除成功',U(GROUP_NAME .'/Travel/detailslist'));	
		}
	}