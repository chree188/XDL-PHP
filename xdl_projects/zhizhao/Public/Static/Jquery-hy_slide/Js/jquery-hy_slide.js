	...
	$.fn.hy_slide = function(options) {
		...
		plugin.getEffect = {
			...
			scrollX : function() {
				...
				$objCon.parent().parent().find('.des_content').html('<a href="'+$objCon.find('li:eq('+indexNext+')').find('a').attr('href')+'" target="_blank" class="a-white-n">'+$objCon.find('li:eq('+indexNext+')').find('a').attr('title')+'</a>');
				index = indexNext;
				plugin.getTrigger();
			},
			//垂直循环式滚动
