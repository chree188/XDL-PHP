...
class ListController extends HomeController
{
    ...
    private function getListBanner(){
        $objAd = M('ad');
        $banner = $objAd->where("id='3' and status='1'")->getField('ad_pic');
        $banner = deep_htmlspecialchars_decode($banner);        //深度转义去字符串的双引号
        return $banner;
    }
    //处理最佳支招
