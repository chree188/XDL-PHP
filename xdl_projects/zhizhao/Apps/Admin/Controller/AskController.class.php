...
class AskController extends CommonController
{
    ...
    public function sortList(){
        ...
        $sortList = M('ask_sort')->order('sequence asc')->select();
    ...
    public function runAddSort(){
        ...
        if(M('ask_sort')->add($_POST)){
            $this->success('添加成功',U(MODULE_NAME  . '/Ask/sortlist'));
        }else{
            $this->error('添加失败');
        }
    }
    ...
    public function editSort(){
        ...
        if($_POST){
            ...
            $result = $db->save($data);
            if($result){
                $this->success('操作成功',U(MODULE_NAME  .'/Ask/sortlist'));
            }else{
                $this->error('操作失败');
            }
        }else{
    ...
    public function deleteSort(){
        $id = I('id',0,'intval');
        $idList = parent::getIdList('ask_sort',$id);
        if($db = M('ask_sort')->where(array('id in('.$idList.')'))->delete()){
            $this->success('删除成功',U(MODULE_NAME  .'/Ask/sortlist'));
        }else{
            $this->error('删除失败');
        }
    }
    ...
    public function sortDetail(){
        ...
        $list = $objAsk->alias('AS a')->join('zz_ask_sort AS s ON a.sort_id = s.id')->field('s.sort_name,a.*')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function detailsList(){
        ...
        $idList = $sort_id.','.$idList; //把当前的分类id 连接起子分类id
        $idList = rtrim($idList,','); //去除右边的 逗号
        $count = $objAsk->count(); //统计总数
        $Page = new Page($count,15); //实例化分页类 传入总记录数和每页显示的记录数
        $list = $objAsk->alias('AS a')->join('zz_ask_sort AS s ON a.sort_id = s.id')->field('s.sort_name,a.*')->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $list = deep_htmlspecialchars_decode($list);
        $this->assign('list',$list);
        $show = $Page->show(); //分页显示输出
        $this->assign('page',$show);
    ...
    public function addDetails(){
        $sortList = M('ask_sort')->select();//先把图文分类读出来
        $sortList = deep_htmlspecialchars_decode($sortList); //查询输出了数据库数据之后记得反转义
    ...
    public function runAddDetails(){
        $db = M('ask');
        parent::checkField('ask_name', deep_htmlspecialchars($_POST['ask_name']), 'require', '图文名称不能为空');
        parent::checkField('content', deep_htmlspecialchars($_POST['content']), 'require', '内容不能为空');
        $data=array();
        $data['ask_name'] = I('ask_name');
        ...
        if($db->data($data)->add()){
            $this->success('添加成功',U(MODULE_NAME .'/Ask/detailslist'));
        }else{
            $this->error('添加失败');
        }
    }
    ...
    public function editDetails(){
        if($_POST){
            ...
            $data=array();
            $data['id'] = $id;
            $data['ask_pic'] = I('ask_pic');
            $data['status'] = I('status');
            $data['tags'] = I('tags');
            if($db->data($data)->save()){
                $this->success('修改成功',U(MODULE_NAME .'/Ask/detailslist'));
            }else{
                $this->error('修改失败');
            }
        }else{
            $id = I('id',0,'intval');
            $this->assign('id',$id);
            $data = M('ask')->where(array('id'=>$id))->field('id,ask_name,content,tags,sort_id,add_time')->find();
    ...
    public function deleteDetails(){
        ...
        if ($_POST) {
            $objAsk->where('`id` in ('.$idList.')')->delete();
        }
        $this->success('删除成功',U(MODULE_NAME  .'/Ask/detailslist'));
    }
}