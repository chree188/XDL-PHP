<?php 
	...
	$sql = 'select i.icon, g.name, g.price, g.desc, g.id, g.sales
	...
	if(empty($gid)){
		$sql = 'select i.icon, g.name, g.price, g.sales, g.look, i.gid, d.detail1, d.detail2, d.detail3
	...
	// $sql = 'select detail1, detail2, detail3
	// 		from goodsDetail  
	// 		where gid = '. ;
	// echo ($sql);
 ...
 <head>
 	...
 	<link rel="stylesheet" href="<?= CSS?>cake-all.css">
 </head>
 <body>
 <!--面包线-->
 	<div id="bread-crumbs">
...
<!--  				<span><a href="">慕斯蛋糕</a></span>
 				...
 				<?php foreach($cate_list as $v): ?>
					<span><a href="cake-all.php?gid=<?=$v['id']?>"><?= $v['name']?></a></span>
				<?php endforeach; ?>
 			</dd>
 		</dl>
 		<div class="clear"></div>
 			...
 			<ul>
 				<?php foreach ($goods_list as $k => $v):?>
	 				<li class="a">
						<!--商品图片-->
	 					<div class="goods-pic">
	 						<a href="<?= URL?>home/action.php?bz=look&id=<?=$v['gid']?>">
	 					...
	 					<div class="goods-action">
	 						<div class="goods-buy">
	 							<a href="<?= URL?>home/goods-details-pages.php?id=<?=$v['gid']?>" class="detail">
		 							<div>
