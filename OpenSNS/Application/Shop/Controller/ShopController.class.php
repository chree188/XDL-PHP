...
class ShopController extends AdminController
{
    ...
    public function goodsEdit($id = 0, $goods_name = '', $goods_ico = '', $goods_introduct = '', $goods_detail = '', $money_need = '', $goods_num = '', $status = '', $category_id = 0, $is_new = 0, $sell_num = 0)
    {
        ...
        if (IS_POST) {
            ...
            $builder->keyId()->keyText('goods_name', L('_GOODS_NAME_'))->keySingleImage('goods_ico', L('_GOODS_BRAND_'))->keySelect('category_id',L('_GOODS_CATEGORY_'), '', $options)->keyText('goods_introduct', L('_GOODS_SLOGAN_'))->keyEditor('goods_detail', L('_GOODS_DETAIL_'))
    ...
    public function goodsBuySuccess($page = 1, $r = 20)
    {
        ...
        $builder->buttonDisable(U('setGoodsBuyStatus'), L('_DELIVER_CANCEL_'))
            ->keyId()->keyText('goods_name', L('_GOODS_NAME_'))->keyUid()->keyText('name', L('_RECEIVER_NAME_'))->keyText('address', L('_RECEIVER_ADDRESS_'))->keyText('zipcode', L('_POST_CODE_'))->keyText('phone', L('_PHONE_NUMBER_'))->keyCreateTime('createtime', L('_BUY_TIME_'))->keyTime('gettime', L('_TRADE_ACCOMPLISH_TIME_'))
            ->data($list)
            ->pagination($totalCount, $r)
            ->display();
    }
