        ...
        if (pkgList.length > 0) {
            ...
            for (var k in expression) {
                if(expression[k].type=='add_img'&&expression[k].title=='add_img'){
                    _imgHtml+='<div  style="display: inline; "class="imghtml" ><img src="' + expression[k].src + '" style="width: 60px;height: 60px;cursor: pointer" onclick="up()"><input type="file" name="myexp" id="myexp" style="display: none" onchange="clicksub(this)"></div>';
                }else {
                    if( expression[k].type=='face'){
                        _imgHtml += '<a href="javascript:void(0)" data-type="' +expression[k].title + '" title="' + expression[k].type  + '" onclick="face_chose($(this))";><img src="' + expression[k].src + '" width="60" height="60" /></a>  ';
                    }else{
            ...
            var count=Math.ceil(expression[k].totalCount/24); if(count>=2) {
...
function postpage(obj){
    ...
    $.post(U('Core/Expression/getSmile'),{pkg:pkg,page:page},function(res) {
        ...
        if (pkgList.length > 0) {
            ...
            var count = Math.ceil(expression[k].totalCount / 24);
            if (count >= 2) {
                _imgHtml += '<div style="text-align: center">';
                for (var l = 1; l < count + 1; l++) {
                    if(l==page){
...
var message_session={
    message_tag:'',
    message_modal_tag:'',
    init_message:function($message_tag,$message_modal_tag){
        message_session.message_tag=$message_tag;
        message_session.message_modal_tag=$message_modal_tag;
        var list_height=message_session.message_tag.height()-message_session.message_tag.find('>.top').height();
        message_session.message_tag.find('.content-list').css('height',list_height);
        ...
        $('[data-role="open-slider-box"]').click(function () {
            $.post(U('Ucenter/Message/messageSession'),{},function(html){
                message_session.message_tag.find('.content-list .session-list').html(html);
                message_session.message_modal_tag.find('.message-list').html();
            });
            message_session.message_modal_tag.find('.close').click();
            $('#message-box .message-slider-box').addClass('open');
        });
        $('[data-role="close-message-slider-box"]').unbind();
        $('[data-role="close-message-slider-box"]').click(function(){
            $('#message-box .message-slider-box').removeClass('open');
        });
    },
    message_list_show_block:function(show_id){
        message_session.message_modal_tag.find('.message-list').find('.list-block').hide();
        $('#'+show_id).show();
    },
        ...
        $('[data-role="open-message-list"]').click(function () {
            ...
            if ($('#message_block_' + session).length == 0) {
                $.post(U('Ucenter/Message/messageDetail'),{message_session:session},function(html){
                    message_session.message_modal_tag.find('.message-list').append(html);
                    $('#message_block_' + session).show();
                    $this.find('.unread-num').hide();
                    $this.find('.unread-tip').hide();
                });
            }
            message_session.message_list_show_block('message_block_' + session);
        });
    },
    init_message_list:function(){
        ...
        $('[data-role="load-more"]').click(function(){
            ...
            $tag.find('.load-more .do-button').html('加载中...');
            var num=5;
