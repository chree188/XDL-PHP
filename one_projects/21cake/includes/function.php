<?php 
	 ...
	 */
	function notice($notice, $url='', $time=2){
		if($url == ''){
		...
		include ROOT.'includes/1.php';
		echo "<div style='width: 500px; height: 100px;position:absolute; left:28%; top:30%;  text-align:center; line-height:80px; color:white;'>".$notice."</div>";
	...
	function page($sql, $num=15){
		...
		if($sum < $num){
			$str="
				<div class='page'>
					<a href='index.php' class='pc'>返回上一级</a>
				</div>
			";
			$result[] = $str;
			return $result;
	...
	function up_img($img_name='icon', $save_path){
			// 上传图片
	...
	function img_root($filename,$width=50){
			$filepath = ROOT.'uploads/';
			$filepath .= substr($filename,0,4).'/';
			$filepath .= substr($filename,4,2).'/';
			$filepath .= substr($filename,6,2).'/';
			$filepath .= $width.'_'.$filename;
