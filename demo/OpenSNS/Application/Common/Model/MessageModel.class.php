...
class MessageModel extends Model
{
     ...
     * @author 郑钟良<zzl@ourstu.com>
     */
    public function getReadMessage($map, $start = 0, $num = 5)
    {
        $map['is_read'] = 1;
            ...
            foreach ($model_message_session as $key => $val) {
                ...
                foreach ($val as $one_type) {
                    ...
                    if($one_type['logo']==null){//模块会话使用模块名logo作为会话logo
                        $one_type['logo'] = APP_PATH . $key . '/Static/images/message_logo/'.strtolower($key).'.png';
                    }else{
                        $one_type['logo'] = APP_PATH . $key . '/Static/images/message_logo/' . $one_type['logo'];
                    }
