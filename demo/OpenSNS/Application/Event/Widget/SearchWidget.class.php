...
class SearchWidget extends Controller
{
    ...
    private function assignEvent()
    {
        ...
        if ($type == 0) {
            $list = M('Event')->where(array('status' => 1, 'eTime' => array('gt',time()), 'title' => array('like', '%' . $keywords . '%')))->limit($num)->order($order)->select();
        } else {
            $list = M('Event')->where(array('status' => 1, 'eTime' => array('gt',time()),'is_recommend' => 1, 'title' => array('like', '%' . $keywords . '%')))->limit($num)->order($order)->select();
