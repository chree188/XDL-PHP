...
function collect(obj){
    var expsrc=$(obj).parent().parent().parent().children().children().attr('src');
    ...
    $.ajaxFileUpload(
        {
            ...
            success: function (a) {
                switch(a){
                    ...
                    default : var html= '<a href="javascript:void(0)" data-type="' + a['name'] + '" title="' + a['from'] + '" onclick="face_chose($(this))";><img src="' + a['path'] + '" width="60" height="60" /></a>  ';$('.imghtml').after(html);toast.success('添加成功!');
                }
            },
            error: function () {
                toast.error('添加失败!');
