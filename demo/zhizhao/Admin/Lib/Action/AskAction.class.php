	...
	class AskAction extends CommonAction{
		...
		public function runAddSort(){
			...
			if(M('ask_sort')->add($_POST)){
				$this->success('添加成功',U(GROUP_NAME . '/Ask/sortlist'));	
			}else{
				$this->error('添加失败');	
			}
		}
		...
		public function editSort(){
			...
			if($_POST){
				...
				$result = $db->save($data);
				if($result){
					$this->success('操作成功',U(GROUP_NAME .'/Ask/sortlist'));
				}else{
					$this->error('操作失败');	
				}
			}else{
		...
		public function deleteSort(){
			...
			if($db = M('ask_sort')->where(array('id in('.$idList.')'))->delete()){
				$this->success('删除成功',U(GROUP_NAME .'/Ask/sortlist'));
			}else{
				$this->error('删除失败');
			}
		}
		...
		public function sortDetail(){
			...
			$list = $objAsk->alias('AS a')->join('yrq_ask_sort AS s ON a.sort_id = s.id')->field('s.sort_name,a.*')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();
...
/*-------------------------------------------------以上分类页  以下详情页----------------------------------------------------*/
		...
		public function detailsList(){
			...
			$Page = new Page($count,15); //实例化分页类 传入总记录数和每页显示的记录数
			$list = $objAsk->alias('AS a')->join('yrq_ask_sort AS s ON a.sort_id = s.id')->field('s.sort_name,a.*')->limit($Page->firstRow.','.$Page->listRows)->select();
			$list = deep_htmlspecialchars_decode($list);
			$this->assign('list',$list);
			$show = $Page->show(); //分页显示输出
			$this->assign('page',$show);
		...
		public function runAddDetails(){
			...
			if($db->data($data)->add()){
				$this->success('添加成功',U(GROUP_NAME.'/Ask/detailslist'));	
			}else{
				$this->error('添加失败');	
			}
		}
		...
		public function editDetails(){
			if($_POST){
				...
				if($db->data($data)->save()){
					$this->success('修改成功',U(GROUP_NAME.'/Ask/detailslist'));	
				}else{
					$this->error('修改失败');	
				}
			}else{
		...
		public function deleteDetails(){
			...
			$this->success('删除成功',U(GROUP_NAME .'/Ask/detailslist'));	
		}
	}