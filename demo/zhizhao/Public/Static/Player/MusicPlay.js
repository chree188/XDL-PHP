...
_MusicPlay.prototype = {
	create: function(options){
		var html = '<object id="Player-'+options.id+'" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0" width="0" height="0">'+
				   '\n<param name="movie" value="'+options.swf+'" />'+
				   '\n<param name="quality" value="high" />'+
				   '\n<param name="allowScriptAccess" value="always" />'+
				   '\n<embed name="Player-'+options.id+'" allowScriptAccess="always" src="'+options.swf+'" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="0" height="0"></embed>'+
				   '\n</object>',
		id = document.getElementById(options.id);
		$('body').append(html);
		//id.style.display='none';
		//id.style.width=0;
		//id.style.height=0;
		//id.style.overflow='hidden';
		this.player = window.document['Player-'+options.id] || document.getElementById('Player-'+options.id);
		
		this.playerUI.init(options);
	},
	play: function(url){
	...
	playerUI : {
		...
		playBtnEvent: function(options){
			...
			$(control.PlayBtn).live('click', function(){
				var title = $(this).attr('title');
				if (title == '播放') {
					$('#CurTime').html() == '00:00' ? MusicPlay.play(options.url) : MusicPlay.rePlay();
					$(this).attr('title', '暂停').addClass('pause');
				} else {
					MusicPlay.pause();
					$(this).attr('title', '播放').removeClass('pause').addClass('play');
				}
			});
