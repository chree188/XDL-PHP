...
<li class='mycollect <?php if(ACTION_NAME == 'collect'): ?>cur<?php endif; ?>'> <a href="<?php echo U(APP_NAME .'/Member/collect');?>">我的金币</a> </li>
    ...
    <div id="right">
    	<div class="artcileBox">
        	<div class="artcileTitle"><h4>提问管理（共<?php echo ($articleAllNum); ?>篇）</h4></div>
            <div class="article_tab">
            	<div class="article_tab_title none">
                	<ul>
                    	<li <?php if(in_array(($_GET['sort']), explode(',',""))): ?>class="current"<?php endif; ?>><a href="<?php echo U(APP_NAME.'/Member/article');?>">全部</a></li>
                        <li <?php if(in_array(($_GET['sort']), explode(',',"1"))): ?>class="current"<?php endif; ?>><a href="<?php echo U(APP_NAME.'/Member/article',array('sort'=>1));?>">生活</a></li>
                        <li <?php if(in_array(($_GET['sort']), explode(',',"2"))): ?>class="current"<?php endif; ?>><a href="<?php echo U(APP_NAME.'/Member/article',array('sort'=>2));?>">情感</a></li>
                        <li <?php if(in_array(($_GET['sort']), explode(',',"3"))): ?>class="current"<?php endif; ?>><a href="<?php echo U(APP_NAME.'/Member/article',array('sort'=>3));?>">健康</a></li>
                        <li <?php if(in_array(($_GET['sort']), explode(',',"3"))): ?>class="current"<?php endif; ?>><a href="<?php echo U(APP_NAME.'/Member/article',array('sort'=>3));?>">职场</a></li>
                        <li <?php if(in_array(($_GET['sort']), explode(',',"3"))): ?>class="current"<?php endif; ?>><a href="<?php echo U(APP_NAME.'/Member/article',array('sort'=>3));?>">公益</a></li>
                    </ul>
                </div>
                <div class="article_tab_con">
                	<!--调十二条-->
                	<ul>
                    	<?php if(is_array($articleList)): $i = 0; $__LIST__ = $articleList;if( count($__LIST__)==0 ) : echo "暂无文章" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><li><a href="__APP__/List/detail/id/<?php echo ($va["id"]); ?>.html" title="<?php echo ($va["ask_name"]); ?>" target="_blank"><?php echo ($va["ask_name"]); ?></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="__APP__/List/editAsk/id/<?php echo ($va["id"]); ?>.html">[编辑]</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="date"><?php echo (date("Y-m-d H:i:s",$va["add_time"])); ?></span></li><?php endforeach; endif; else: echo "暂无文章" ;endif; ?>
                    </ul>
                </div>
                <div class="fpage">
                	<?php echo ($page); ?>
